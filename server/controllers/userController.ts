...
import client from '../configs/openai';
...
// Background function that does the actual AI generation.
// Runs AFTER the response has already been sent to the client,
// so it must NEVER touch `res`.
async function generateWebsiteContent(projectId: string, userId: string, initial_prompt: string) {
    try {
        // Enhance user prompt
        const promptEnhancerResponse = await client.chat.completions.create({ 
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: `I've enhanced your prompt too: "${enhancedPrompt}"`,
                projectId
            }
        })
        await prisma.conversation.create({
        ...
        const codeGenerationResponse = await client.chat.completions.create({
            model: 'tencent/hy3:free',
            messages: [
                {
                    role: 'system',
    ...
    - Make sure all buttons, cards, and components use Tailwind styling
     ...
     CRITICAL HARD RULES:
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: "Something went wrong while generating your website. Credits refunded.",
                projectId
            }
        });
    }
...
export const createUserProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // Respond immediately with the projectId; frontend polls
        // /api/project/preview/:projectId until current_code is set.
        res.json({ projectId: project.id })

        // Kick off generation in the background. Do NOT await this,
        // and never touch `res` inside it — the response is already sent.
        generateWebsiteContent(project.id, userId, initial_prompt).catch((err) => {
...
// Controller function to get all of a user's projects
